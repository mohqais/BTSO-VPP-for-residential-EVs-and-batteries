...
class bGWO():
    def __init__(self, fit_func,num_dim=30, num_particle=30, max_iter=500, 
                 a_max=2, a_min=0):
        ...
        self.X=np.zeros((num_particle,num_dim))

        self.X = 1*(np.random.uniform(size=[self.num_particle, self.num_dim])>0.7)
